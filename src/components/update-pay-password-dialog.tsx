...
import { updatePayPassword } from 'src/api/user';
...
type UpdatePayPasswordDialogProps = {
  open: boolean;
  onClose: () => void;
};

export function UpdatePayPasswordDialog({ open, onClose }: UpdatePayPasswordDialogProps) {
  const { t } = useLanguage();
  const [loading, setLoading] = useState(false);
  const userInfo = useAuthStore((s) => s.userInfo);

  const payPasswordSchema = z
    .object({
  ...
  type PayPasswordForm = z.infer<typeof payPasswordSchema>;

  const methods = useForm<PayPasswordForm>({
  ...
  const onSubmit = methods.handleSubmit(async (values) => {
    ...
    setLoading(true);
    try {
      const res = await updatePayPassword({
        id: userInfo.id,
        pwd: values.pwd,
        rePwd: values.rePwd,
      ...
      if (res.code === '1' || res.code === 1) {
        toast.success(t('password.payPasswordUpdateSuccess'));
        methods.reset();
        onClose();
      } else {
  ...
  return (
    ...
    >
      <DialogTitle>{t('password.updatePayPassword')}</DialogTitle>

      <DialogContent dividers sx={{ pt: 3 }}>
        <DialogContentText sx={{ mb: 2 }}>{t('password.updatePayPasswordDesc')}</DialogContentText>
