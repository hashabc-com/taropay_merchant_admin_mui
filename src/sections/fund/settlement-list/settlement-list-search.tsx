...
export function SettlementListSearch() {
  const { t } = useLanguage();
  ...
  return (
    <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 1.5, mb: 3 }}>
      ...
      <FormControl size="small" sx={{ width: 130 }}>
        <InputLabel shrink>{t('fund.settlement.type')}</InputLabel>
        <Select
          displayEmpty
          label={t('fund.settlement.type')}
          notched
          value={values.type}
          onChange={(e) => setField('type', e.target.value)}
          renderValue={(sel) => {
            if (!sel) return <span style={{ color: '#aaa' }}>{t('common.pleaseSelect')}</span>;
            return TYPE_OPTIONS.find((o) => o.value === sel)?.label || sel;
          }}
        >
      ...
      <FormControl size="small" sx={{ width: 130 }}>
        <InputLabel shrink>{t('fund.settlement.auditStatus')}</InputLabel>
        <Select
          displayEmpty
          label={t('fund.settlement.auditStatus')}
          notched
          value={values.status}
